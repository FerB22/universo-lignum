# Script to assemble interactive single-page HTML for "Ruk el Héroe"

$editedDir = "c:\Users\Barra\Documents\Ruk - Historia Completa\Ruk_Editado"
$outputHtml = "c:\Users\Barra\Documents\Ruk - Historia Completa\index.html"

# Chapter files
$chapterFiles = @(
    "Capitulo_01_El_peso_de_la_partida.md",
    "Capitulo_02_Refugio_y_promesa.md",
    "Capitulo_03_El_frente_de_Ende.md",
    "Capitulo_04_Manantial_de_sanacion.md",
    "Capitulo_05_La_marea_enemiga.md",
    "Capitulo_06_Rutas_de_suministros.md",
    "Capitulo_07_Forjando_guerreros.md",
    "Capitulo_08_La_union_de_las_tribus.md",
    "Capitulo_09_Heroes_de_Davir.md"
)

$chapterTitles = @(
    "Capítulo 1: El peso de la partida",
    "Capítulo 2: Refugio y promesa",
    "Capítulo 3: El frente de Ende",
    "Capítulo 4: Manantial de sanación",
    "Capítulo 5: La marea enemiga",
    "Capítulo 6: Rutas de suministros",
    "Capítulo 7: Forjando guerreros",
    "Capítulo 8: La unión de las tribus",
    "Capítulo 9: Héroes de Davir"
)

# Read chapters and build HTML content for each
$chaptersHtmlList = @()
$chapterIndex = 1

foreach ($file in $chapterFiles) {
    $filePath = Join-Path $editedDir $file
    if (Test-Path $filePath) {
        $rawText = [System.IO.File]::ReadAllText($filePath, [System.Text.Encoding]::UTF8)
        
        # Remove top markdown header line like # Capítulo X: ...
        $lines = $rawText -split "\r?\n"
        $filteredLines = @()
        foreach ($l in $lines) {
            if ($l -notmatch "^\s*#\s*Capítulo" -and $l -notmatch "^\s*---\s*$") {
                $filteredLines += $l
            }
        }
        $cleanText = $filteredLines -join "`n"
        ...
        $bodyContent = $pBlocks -join "`n"
        $title = $chapterTitles[$chapterIndex - 1]
        
        $activeClass = if ($chapterIndex -eq 1) { "active-chapter" } else { "" }
        
        $chHtml = @"
        <article class="chapter-content $activeClass" id="capitulo-$chapterIndex" data-chapter="$chapterIndex">
            <div class="chapter-header">
                <span class="chapter-badge">✦ CAPÍTULO 0$chapterIndex ✦</span>
            ...
            <div class="chapter-nav-bottom">
                $(if ($chapterIndex -gt 1) { "<button class='btn-nav-ch' onclick='switchChapter($($chapterIndex - 1))'>← Capítulo Anterior</button>" } else { "<span></span>" })
                <button class="btn-nav-ch btn-gold-fill" onclick="window.scrollTo({top:0, behavior:'smooth'})">↑ Volver Arriba</button>
                $(if ($chapterIndex -lt 9) { "<button class='btn-nav-ch' onclick='switchChapter($($chapterIndex + 1))'>Capítulo Siguiente →</button>" } else { "<span></span>" })
            </div>
        </article>
"@
        $chaptersHtmlList += $chHtml
    }
    $chapterIndex++
}

$allChaptersHtml = $chaptersHtmlList -join "`n`n"
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Rule: Sangría de primera línea 0.75em */
        .chapter-text-body p {
            text-indent: 0.75em;
            margin-bottom: 1.3em;
        }
...
<body>
        ...
        <main class="main-container" id="menu-principal">
            ...
            <div class="hub-grid">
                ...
                <div class="btn-hub-card" onclick="mostrarSeccion('seccion-actos', 'actos')">
                    ...
                    <p class="hub-card-desc">Recorre la cronología escénica del ascenso de los héroes y las batallas decisive.</p>
                </div>
    ...
    <script>
        // Transición de vistas exclusivas (guia_transicion_vistas.md)
        function mostrarSeccion(idTarget, hash) {
            const menu = document.getElementById('menu-principal');
            const target = document.getElementById(idTarget);

            // 1. Ocultar otras secciones abiertas
            ...
            // 2. Animar salida del menú principal
            menu.classList.add('closing');

            // 3. Esperar 250ms a que termine fadeOut
            setTimeout(() => {
                menu.classList.add('hidden');
                menu.classList.remove('closing');

                // 4. Mostrar sección objetivo con fadeIn
...
[System.IO.File]::WriteAllText($outputHtml, $htmlTemplate, [System.Text.UTF8Encoding]::new($false))
Write-Host "Successfully generated $outputHtml with all 9 chapters!"
